<x-layout title="Galeri Desa">
                ...
                <div class="group relative overflow-hidden rounded-lg shadow-sm hover:shadow-md transition-all duration-300 bg-white">
                    <div class="aspect-w-4 aspect-h-3">
                        @if($g->gambar)
                            <img src="{{ asset('storage/' . $g->gambar) }}" 
                                 alt="{{ $g->keterangan_gambar ?? 'Galeri' }}" 
                                 class="w-full h-64 object-cover transform group-hover:scale-105 transition-transform duration-500">
                        @else
                            <div class="w-full h-64 flex items-center justify-center bg-gray-200 text-gray-400">Tidak ada gambar</div>
                        @endif
                    </div>
                    <div class="absolute inset-0 bg-gradient-to-t from-gray-900/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                        <div class="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                            ...
                            <p class="text-gray-300 text-sm mt-1">{{ $g->tanggal }}</p>
                        </div>
                    </div>
                    <span class="absolute top-3 right-3 bg-gray-800/90 text-white text-xs px-3 py-1 rounded-full uppercase tracking-wider">{{$g->kategori}}</span>
                </div>
                @endforeach
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        @foreach ($galeri as $g)
            ...
            @if ($g->link_video && is_string($g->link_video))
                @php
                    $youtubeId = null;
                    // Ekstrak ID YouTube dari berbagai format URL
                    $pattern = '/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|youtu\.be\/|youtube\.com\/watch\?v=)([^"&?\/\s]{11})/';
                    if (preg_match($pattern, $g->link_video, $matches)) {
                        $youtubeId = $matches[1];
                        $hasVideos = true;
                    }
                @endphp
                ...
                @if ($youtubeId)
                    <div class="bg-white rounded-lg overflow-hidden shadow-md border border-gray-200">
                        <div class="relative pb-[56.25%] h-0"> <iframe 
                        ...
                        <div class="p-4">
                            <h3 class="font-medium text-gray-800 mb-1">{{ $g->keterangan_video ?? 'Video Dokumentasi Desa' }}</h3>
