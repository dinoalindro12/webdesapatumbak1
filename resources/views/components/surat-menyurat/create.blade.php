                ...
                <form method="POST" action="{{ route('layanan.surat-menyurat.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="mb-6">
                            ...
                            <select id="jenis_surat" name="jenis_surat" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-800 @error('jenis_surat') border-red-500 @enderror">
                                <option value="" disabled {{ old('jenis_surat') ? '' : 'selected' }}>-- Pilih Jenis Surat --</option>
                                <option value="ktp" {{ old('jenis_surat')=='ktp' ? 'selected' : '' }}>Pembuatan KTP</option>
                                <option value="kk" {{ old('jenis_surat')=='kk' ? 'selected' : '' }}>Pembuatan Kartu Keluarga (KK)</option>
                                <option value="sk_domisili" {{ old('jenis_surat')=='sk_domisili' ? 'selected' : '' }}>Surat Keterangan Domisili</option>
                                <option value="sk_ck" {{ old('jenis_surat')=='sk_ck' ? 'selected' : '' }}>Surat Keterangan Catatan Kriminal</option>
                                ...
                                <option value="sk_kematian" {{ old('jenis_surat')=='sk_kematian' ? 'selected' : '' }}>Surat Keterangan Kematian</option>
                            </select>
                            @error('jenis_surat')<div class="text-red-500 text-xs mt-1">{{ $message }}</div>@enderror
                        </div>
                        <!-- Alasan -->
                        <div class="mb-6">
                            <label for="alasan" class="block text-sm font-medium text-gray-700 mb-1">Alasan Permohonan</label>
                            <textarea id="alasan" name="alasan" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 @error('alasan') border-red-500 @enderror" placeholder="Jelaskan tujuan pembuatan surat ini">{{ old('alasan') }}</textarea>
                            @error('alasan')<div class="text-red-500 text-xs mt-1">{{ $message }}</div>@enderror
                        </div>
                        <!-- Upload Dokumen -->
                        <div>
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="px-6 py-2 bg-gray-800 text-white font-medium rounded-md hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                            Ajukan Permohonan
                        </button>
                    </div>
                </form>
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                ...
                <div class="mt-4 p-3 bg-blue-100 border border-blue-200 rounded-md">
                    <p class="text-sm text-white-500 flex items-start">
                        ...
                        Untuk pertanyaan lebih lanjut, silahkan hubungi kantor desa di jam kerja (08.00-15.00) atau melalui WhatsApp di 0812-3456-7890
                    </p>
                </div>
            </div>
        </div>
