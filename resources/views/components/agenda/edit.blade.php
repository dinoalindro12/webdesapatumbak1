<x-app-layout title="Edit Proyek Desa">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white shadow rounded-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-2xl font-serif font-light text-gray-800">Edit Proyek Desa</h2>
            </div>
            <form action="{{ route('agenda.update', $agenda->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_proyek" id="nama_proyek" value="{{ old('nama_proyek', $agenda->nama_proyek) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="waktu_mulai" id="waktu_mulai" value="{{ old('waktu_mulai', $agenda->waktu_mulai) }}"
                            min="1900" max="{{ date('Y') }}"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm">
                        @error('waktu_mulai')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="besar_anggaran" id="besar_anggaran" value="{{ old('besar_anggaran', $agenda->besar_anggaran) }}"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="text" name="ukuran_proyek" id="ukuran_proyek" value="{{ old('ukuran_proyek', $agenda->ukuran_proyek) }}"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm">
                        @error('ukuran_proyek')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="Berjalan" {{ old('status', $agenda->status) == 'Berjalan' ? 'selected' : '' }}>Berjalan</option>
                            <option value="Tertunda" {{ old('status', $agenda->status) == 'Tertunda' ? 'selected' : '' }}>Tertunda</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
